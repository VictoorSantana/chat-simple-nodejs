...
router.get('/', request(), async function (req, res) {
    try {
        const response = await userService.find(req.query, req.user);
...
router.post('/', validate(userRequest), async function (req, res) {
    try {
        const response = await userService.create(req.value, req.files);
        res.status(201).send(response);
    } catch (e) {
        console.error(e);
        res.status(401).send(e.message);
...
router.put('/:id', request(userRequest), async function (req, res) {
    try {
        const response = await userService.update(req.params.id, req.value, req.files);
        res.status(201).send(response);
    } catch (e) {
        console.error(e);
        res.status(401).send(e.message);
